<header class="dashboard-head float-start w-100">
    ...
    <div class="d-none d-lg-flex w-100 top-header align-items-center justify-content-start">
     ...
     @php
     $data = \DB::table('members')->where('id', session('userid'))->get();
     $token = @$data[0]->device_token;

     if($token=='') { @endphp
      <center>
        <button id="btn-nft-enable" onclick="initFirebaseMessagingRegistration()" class="btn btn-danger btn-xs btn-flat">Allow Notification</button>
     </center>
     @php } @endphp
        ...
        messaging
            ...
            .then(function(token) {
                ...
                $.ajax({
                    ...
                    url: 'https://www.writerzcentercrm.in/save-token',
                    type: 'POST',
                    data: {token:token},
                    dataType: 'JSON',
                    success: function (response) {
                        alert('Notifications allowed successfully.');
                        $("#btn-nft-enable").hide();
                    },
                    error: function (err) {

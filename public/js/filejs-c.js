const fInputs = document.querySelectorAll(".btcd-f-input-c>div>input");
...
function delItem(el) {
  ...
  fInp.files = createFileListc(...fileList.files);
...
for (let inp of fInputs) {
  inp.parentNode.querySelector(".btcd-inpBtn-c>img").src =
...
function handleFile(e) {
  ...
  if (e.target.hasAttribute("multiple")) {
    e.target.files = createFileListc(...fileList.files);
  } else {
    e.target.files = createFileListc(fileList.files[fileList.files.length - 1]);
    fileList = { files: [] };
  }

  // set File list view
  if (e.target.files.length > 0) {
    e.target.parentNode.querySelector(".btcd-f-title-c").innerHTML =
      e.target.files.length + " File Selected";
    e.target.parentNode.parentNode.querySelector(".btcd-files-c").innerHTML = "";
    for (let i = 0; i < e.target.files.length; i += 1) {
      let img = null;

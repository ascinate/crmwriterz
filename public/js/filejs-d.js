const fInputs = document.querySelectorAll(".btcd-f-input-d>div>input");
...
function delItem(el) {
  fileList = { files: [] };
  let fInp = el.parentNode.parentNode.parentNode.querySelector(
    'input[type="file"]'
  );
  ...
  fInp.files = createFileListd(...fileList.files);
...
for (let inp of fInputs) {
  inp.parentNode.querySelector(".btcd-inpBtn-d>img").src =
...
function setPrevData(e) {
  if (e.target.hasAttribute("multiple_d") && fName !== e.target.name) {
    console.log("multiple_d");
    fName = e.target.name;
...
function handleFile(e) {
  ...
  if (e.target.hasAttribute("multiple_d")) {
  ...
  if (e.target.hasAttribute("multiple_d")) {
    e.target.files = createFileListd(...fileList.files);
  } else {
    e.target.files = createFileListd(fileList.files[fileList.files.length - 1]);
    fileList = { files: [] };
  }

  // set File list view
  if (e.target.files.length > 0) {
    e.target.parentNode.querySelector(".btcd-f-title-d").innerHTML =
      e.target.files.length + " File Selected";
    e.target.parentNode.parentNode.querySelector(".btcd-files-d").innerHTML = "";
    for (let i = 0; i < e.target.files.length; i += 1) {
      let img = null;
